...
export default {
  ...
  methods: {
    async submitData() {
      this.dreamTeamId = await IdentityProduct.updateData(this.dreamTeamId);
      this.editDreamTeam = false;
    },
    async fetchData(){
      this.dreamTeamId = await IdentityProduct.fetchData();
      console.log('coucou',this.dreamTeamId);
    },
    editIdProduct: () => {
      this.editDreamTeam = !this.editDreamTeam;
    }
  }
};
...
<template>
  <section>
    <div class="text-end editStyle mb-3">
      <button v-if="isEditable" class="me-5 btn btn-info" @click="editIdProduct">
      ...
      <div class="d-flex flex-wrap flex-lg-nowrap g-3 align-items-center justify-content-center">
        <div class="col-12 col-lg-8 order-1 order-lg-0">
          <div>
            ...
            <img v-if="language === 'fr-FR'" :src="dreamTeamId.pictureFr" v-bind:alt="$t('exempleIdentityPicture')"
              class="col-12 img-fluid bannerPicture">
            <img v-else :src="dreamTeamId.pictureEn" v-bind:alt="$t('exempleIdentityPicture')"
              class="col-12 img-fluid bannerPicture">
          </div>
        </div>
      ...
      <div v-if="editDreamTeam">
        <div class="d-flex flex-wrap align-items-center justify-content-center mx-5 mb-3">
          ...
          <div class="col-12 col-lg-4 d-flex">
            <input type="text" v-model="dreamTeamId.pictureEn" class="form-control mt-2"
              v-bind:placeholder="$t('uploadNewimage')" required>
            <i class="h3 m-2 bi bi-filetype-jpg"></i>
          </div>
          <label>{{ $t('english') }}</label>
          <div class="col-12 col-lg-4 mx-3">
            <input v-model="dreamTeamId.sloganEn" :placeholder="dreamTeamId.sloganEn" class="form-control mt-2">
          </div>
        </div>
        <div class="d-flex flex-wrap align-items-center justify-content-center mx-5 mb-3">
          <label>{{ $t('french') }}</label>
          <div class="col-12 col-lg-4 d-flex">
            <input type="text" v-model="dreamTeamId.pictureFr" class="form-control mt-2"
              v-bind:placeholder="$t('uploadNewimage')" required>
            <i class="h3 m-2 bi bi-filetype-jpg"></i>
          </div>
          <label>{{ $t('french') }}</label>
          <div class="col-12 col-lg-4 mx-3">
            <input v-model="dreamTeamId.sloganFr" :placeholder="dreamTeamId.sloganFr" class="form-control mt-2">
          </div>
        </div>
